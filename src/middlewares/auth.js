...
const tokenCheck = async (req, res, next) => {
    ...
    if (!headerToken)
        throw new APIError("Nädogry giriş. Täzeden synanşyň",401)

    const token = req.headers.authorization.split(" ")[1]

    await jwt.verify(token, process.env.JWT_SECRET_KEY, async (err, decoded) => {
        if (err) throw new APIError("Nädogry Token",401)

        const userInfo = await user.findById(decoded.sub).select("_id name lastname email ")

        if (!userInfo)
            throw new APIError("Nädogry Token",401)
...
const decodedTemporaryToken = async (temporaryToken) => {
    ...
    await jwt.verify(token, process.env.JWT_TEMPORARY_KEY, async (err, decoded) => {
        ...
        if (!userInfo) throw new APIError("Nädogry Token", 401)
